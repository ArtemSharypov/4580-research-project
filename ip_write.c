...
size_t data_len;
{
	...
	if (proto == IPPROTO_UDP)
	{
		...
		acc_t *ip_hdr_acc= bf_cut(data, 0, IP_MIN_HDR_SIZE);
		...
		acc_t *udp_acc= bf_cut(data, ip_hdr_size, UDP_HDR_SIZE); 
	...
	{
		acc_t *dup_data = bf_dupacc(data);

		// Taken from tcp.c for the following logic :(
		ip_hdr_t *ip_hdr= (ip_hdr_t *)ptr2acc_data(dup_data);
		size_t ip_hdr_len= (ip_hdr->ih_vers_ihl & IH_IHL_MASK) << 2;
		size_t ip_datalen= data_len - ip_hdr_len;
		...
		size_t tcp_hdr_len= (tcp_hdr->th_data_off & TH_DO_MASK) >> 2;
	...
	if (should_block_outgoing_packet(proto, src_port, dest_port, ip_hdr->ih_src, ip_hdr->ih_dst))
	{
		bf_afree(data);
		return NW_OK;
	}
