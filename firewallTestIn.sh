...
readonly TEST_PATH=/usr/src/minix/commands/minifirewall/testResults/testTerminalOutputIn.txt
...
echo "---------------------------------------------------------------------------"
printf "Testing traffic manipulation of incoming packets\n" &>> $TEST_PATH
printf "\n Progress...................0/100\n" $>> $TEST_PATH
printf "\n\n" &>> $TEST_PATH
# Clear policy list
...
printf "\nminifirewall --in --srcip 10.0.2.15 --srcnetmask 255.255.255.255 --proto ALL --action BLOCK\n" &>> $TEST_PATH
minifirewall --in --srcip 10.0.2.15 --srcnetmask 255.255.255.255 --proto ALL --action BLOCK &>> $TEST_PATH
ping -c 1 255.255.255.255 &>> $TEST_PATH
printf "\nminifirewall --delete 1\n" &>> $TEST_PATH
minifirewall --delete 1 &>> $TEST_PATH
###############################################################################################
...
printf "\nminifirewall --in --proto TCP --action BLOCK\n" &>> $TEST_PATH
minifirewall --in --proto TCP --action BLOCK &>> $TEST_PATH
(sleep 5; echo "Succesfully Recieved the TCP message" | nc -w 1 127.0.0.1 80) | nc -w 10 -l -p 80 &>> $EXTRA_LOGS
print_result
printf "\nminifirewall --in --proto TCP --action UNBLOCK\n" &>> $TEST_PATH
minifirewall --in --proto TCP --action UNBLOCK &>> $TEST_PATH
(sleep 5; echo "Succesfully Recieved the TCP message" | nc -w 1 127.0.0.1 80) | nc -w 10 -l -p 80 &>> $EXTRA_LOGS
print_result
printf "\nminifirewall --delete 2\n" &>> $TEST_PATH
minifirewall --delete 2 &>> $TEST_PATH
